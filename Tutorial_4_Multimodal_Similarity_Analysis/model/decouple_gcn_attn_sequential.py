...
class unit_tcn(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=9, stride=1, num_point=49, block_size=41):
...
class unit_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, groups, num_point, coff_embedding=4, num_subset=3, device='cuda'):
        ...
        if device == 'cuda':
            self.eyes = nn.Parameter(torch.stack(eye_array).clone().detach().requires_grad_(False).cuda(), requires_grad=False)  # [c,25,25]
        else:
            self.eyes = nn.Parameter(torch.stack(eye_array).clone().detach().requires_grad_(False), requires_grad=False)
...
class TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, groups, num_point, block_size, stride=1, residual=True, attention=True, device='cuda'):
        super(TCN_GCN_unit, self).__init__()
        num_jpts = A.shape[-1]
        self.gcn1 = unit_gcn(in_channels, out_channels, A, groups, num_point, device=device)
        ...
        self.A = nn.Parameter(torch.tensor(np.sum(np.reshape(A.astype(np.float32), [
                              3, num_point, num_point]), axis=0), dtype=torch.float32, requires_grad=False, device=device), requires_grad=False)
...
class Model(nn.Module):
    def __init__(self, num_class=226, num_point=27, num_person=1, groups=16, block_size=41, graph='graph.sign_27.Graph', graph_args=dict(), in_channels=3, device='cuda'):
        ...
        self.l1 = TCN_GCN_unit(in_channels, 64, A, groups, num_point,
                               block_size, residual=False, device=device)
        self.l2 = TCN_GCN_unit(64, 64, A, groups, num_point, block_size, device=device)
        self.l3 = TCN_GCN_unit(64, 64, A, groups, num_point, block_size, device=device)
        self.l4 = TCN_GCN_unit(64, 64, A, groups, num_point, block_size, device=device)
        self.l5 = TCN_GCN_unit(
            64, 128, A, groups, num_point, block_size, stride=2, device=device)
        self.l6 = TCN_GCN_unit(128, 128, A, groups, num_point, block_size, device=device)
        self.l7 = TCN_GCN_unit(128, 128, A, groups, num_point, block_size, device=device)
        self.l8 = TCN_GCN_unit(128, 256, A, groups,
                               num_point, block_size, stride=2, device=device)
        self.l9 = TCN_GCN_unit(256, 256, A, groups, num_point, block_size, device=device)
        self.l10 = TCN_GCN_unit(256, 256, A, groups, num_point, block_size, device=device)

        self.fc = nn.Linear(256, num_class)
        nn.init.normal_(self.fc.weight, 0, math.sqrt(2. / num_class))
        bn_init(self.data_bn, 1)

    def forward(self, x, keep_prob=0.9, eval=False, **kwargs):
        N, C, T, V, M = x.size()
        ...
        return {
            ...
            "semantic_local_features": None,
            "skeleton_local_features": None,
            "skeleton_predictions": None,
            "one_branch_cross_modal": None,
            "matching_predictions": None,
            'global': x,
            "representations": x
        }
