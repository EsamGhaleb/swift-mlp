...
def get_last_phoneme_timestamp(segment, word, puncts):
    ...
    # the count is to ignore prolonged utterance like "ik..."
    if word.get("word").count(".") > 1:
        last_phoneme_end = word.get("end")
    # if the last phoneme of the word is not a punctuation or in the word2vec dictionary, then this word isn't the last word in the segment
    elif word.get("word")[-1] not in puncts:
        last_phoneme_end = word.get("end")
    # this is to handle the case where the last phoneme is è, which is not in the word2vec dictionary
    elif "è" in word.get("word"):
        last_phoneme_end = word.get("end")
    # if the last or second-to-last character is a number, then set the end time to the end time of the word
    elif word.get("word")[-1].isnumeric() or word.get("word")[-2].isnumeric():
        last_phoneme_end = word.get("end")
    else:
        try:
            ...
            print(
                f"ERROR: The word is not in the segment. Word = {word.get('word')}; Segment = {segment['text']}")
        ...
        try:
            ...
            print(
                f"ERROR: There is no phoneme in the segment. Phoneme = {last_phoneme_dict['char']}; Word = {word.get('word')}; Segment = {segment['text']}")

        # if the duration is longer than 1 second, we will set the end time to 50ms after the start time
        if duration > 1:
            # if the duration is longer than 1 second, we will set the end time to 50ms after the start time
            last_phoneme_end = last_phoneme_start + 0.050
        else:
            # if the duration is less than 1 second, we will set the end time to the end time of the word
            last_phoneme_end = word.get("end")
...
def format_transcript(result, filename, output_folder, puncts, word_spacing=True):
    ...
    for segment in segments:
        ...
        for word in segment["words"]:
            ...
            if text == "" or second_count == 1:
                ...
                segment["start_word_timestamp"] = convert_time_float_to_string(
                    word.get("start"))
                # "end_word" and "end_word_timestamp" will be overwritten if there are multiple words within one segment
                segment["end_word"] = text
                segment["end_word_timestamp"] = get_last_phoneme_timestamp(
                    segment, word, puncts)
                if word.get("word")[-1] not in puncts:
                    previous_endTime = convert_time_float_to_string(
                        word.get("end"))
            ...
            #### Last word ####
                ...
                if word.get("end") is not None:
                    ...
                    segment["end_word_timestamp"] = get_last_phoneme_timestamp(
                        segment, word, puncts)
                    previous_endTime = get_last_phoneme_timestamp(
                        segment, word, puncts)
                else:
            ...
            #### Middle words ####
                ...
                if word.get("word")[-1] in puncts:
                    segment["end_word"] = word.get("word")
                    segment["end_word_timestamp"] = convert_time_float_to_string(
                        word.get("end"))

                    df_output = make_row_for_each_segment(
                        segment, text, df_output)
                    second_count += 1
                    text = ""
                    continue  # skip the rest of the loop and go to the next word

            count += 1
        # if the row for the utterance has not been made yet (because we didn't have to split the utterance)
        if second_count == 0:
    ...
    for index, row in df_output.iterrows():
        df_output.loc[index, 'start'] = convert_time_float_to_string(
            df_output.loc[index, 'start'])
        df_output.loc[index, 'end'] = convert_time_float_to_string(
            df_output.loc[index, 'end'])
...
def export_transcript_as_tsv(result, filename, output_folder, puncts):
    df_output, output_filename = format_transcript(
        result, filename, output_folder, puncts)
    # we only need the following columns
    df_output = df_output[['start_word_timestamp',
                           'end_word_timestamp', 'text_final']]
    # change column names
    df_output = df_output.rename(columns={
                                 "start_word_timestamp": "start", "end_word_timestamp": "end", "text_final": "text"})
