...
class unit_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, groups, num_point, coff_embedding=4, num_subset=3, device='cpu'):
        ...
        self.eyes = nn.Parameter(torch.stack(eye_array).clone().detach().requires_grad_(False).cpu(), requires_grad=False)  # [c,25,25]
